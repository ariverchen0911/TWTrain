...
本專案是花蓮縣光復隧道北口擋土牆溢淹預警監測資訊整合系統的 HTML5 POC，目前版本 **v1.2.0（2026-09-20）**。
...
瀏覽器測試需先啟動上述本機伺服器，預設使用 Windows Chrome。其他環境可用 `GFT_CHROME` 指定 Chromium 執行檔、`GFT_URL` 指定網站。結果與截圖輸出至 `outputs/qa/v1.2.0/`（不提交 Git）。
